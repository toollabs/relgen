<?php
     ...
     * @translation to Spanish by Elisardojm
     */
?>
<!DOCTYPE HTML>
<html>
  ...
  <body style="background: url('bg.png') no-repeat fixed right bottom; overflow:hidden;">
    ...
    <div class="container">
      <h1>Generador de documentos de liberación para Wikimedia OTRS <small><a id="meta" tabindex="0" data-toggle="popover" data-placement="bottom" data-trigger="focus" 
...
M.</a><br />(es translation by Elisardojm)" style="color:#777;"><?=$relgen?></a></small></h1>
...
target="_blank">ayuda</a> | <a href="//commons.wikimedia.org/wiki/User_talk:FDMS4?action=edit&section=new&preloadtitle=Wikimedia+OTRS+release+generator+feedback" 
...
/>Soy el titular de los derechos de autor</label>
              <label class="btn btn-default btn-block" onclick="$('#iam').show();$('#irep').show();$('#idk').hide();"><input type="radio" id="s12" name="s1" value="2" 

/>Represento al titular de los derechos de autor</label>
              <label class="btn btn-default btn-block" onclick="$('#iam').hide();$('#irep').hide();$('#idk').show();"><input type="radio" id="s13" name="s1" value="" 

/>No conozco al titular de los derechos de autor</label>
            </div>
            <br />
            <p>El titular de los derechos de autor de un trabajo multimedia es la <b>persona que lo creó</b> (fotógrafo, diseñador, pintor, ...), a no ser que los 

derechos de autor fueran transferidos explícitamente por unha operación legal o contrato.</p>
            <br /><br />
          </div>
          <div class="col-md-4">
            <div style="display:none;" id="iam">
              mi nombre<br /><div id="s1fg1" class="form-group"><input id="namei" type="text" name="name" value="<?php echo $name;?>" placeholder="Juán Nadie 

(requerido)" class="form-control" /></div>
              <div style="display:none;" id="irep">
                <br />
                titular de los derechos de autor<br /><div id="s1fg2" class="form-group"><input id="repi" type="text" name="rep" value="<?php echo $rep;?>" 

placeholder="Empresa S.A. / Juana Nadie (requerido)" class="form-control" /></div>
                <br />
                mi autoridad<br /><div id="s1fg3" class="form-group"><input id="authi" type="text" name="auth" value="<?php echo $auth;?>" placeholder="Presidente, 

representante autorizado, … (requerido)" class="form-control" /></div>
              </div>
              <br /><br />
            ...
            <div style="display:none;" id="idk">
              <p class="text-danger">Wikimedia OTRS no puede aceptar una liberación de usted – por favor, en lugar de esto <a 

href="//commons.wikimedia.org/wiki/Commons:OTRS#notch">contacte con el titular de los derechos de autor</a>.</p>
            </div>
          </div>
          <br />
        </div>
...
+Wikimedia+OTRS+release+generator+step+2" target="_blank">ayuda</a> | <a href="//commons.wikimedia.org/wiki/User_talk:FDMS4?

action=edit&section=new&preloadtitle=Wikimedia+OTRS+release+generator+feedback" target="_blank">comentarios</a> ]
          <br /><br />
          <div class="col-md-7">
            <div class="button" data-toggle="buttons">
              <label class="btn btn-default btn-block" onclick="$('#iup').show();$('#iatt').show();"><input type="radio" id="s21" name="s2" value="1" />Yo o otros ya 

hemos subido el fichero a Wikimedia Commons</label>
              <label class="btn btn-default btn-block" onclick="$('#iup').hide();$('#iatt').show();"><input type="radio" id="s22" name="s2" value="2" />Voy a anexar el 

fichero al correo electrónico</label>
            </div>
            <br />
            <p>Por favor, use el <a href="//commons.wikimedia.org/wiki/Special:UploadWizard" target="_blank"><b>Asistente de cargas</b></a> para subir el fichero a 

Wikimedia Commons si aún no lo hizo.</p>
            <p>Para evitar que el fichero sea borrado mientras su correo de liberación espera a ser procesado por el equipo de OTRS de Wikimedia, usted puede añadir 

<b><samp>{{subst:OP}}</samp></b> en la página de descripción del fichero.</p>
            <br /><br />
          </div>
          <div class="col-md-4">
...
+Wikimedia+OTRS+release+generator+step+3" target="_blank">ayuda</a> | <a href="//commons.wikimedia.org/wiki/User_talk:FDMS4?

action=edit&section=new&preloadtitle=Wikimedia+OTRS+release+generator+feedback" target="_blank">comentarios</a> ]
          <br /><br />
          <div class="col-md-7">
            <div class="button" data-toggle="buttons">
              <label class="btn btn-default btn-block" onclick="$('#i3').show();"><input type="radio" id="s31" name="s3" value="1" />Quiero liberar el trabajo 

multimedia</label>
              <label class="btn btn-default btn-block" onclick="$('#i3').show();"><input type="radio" id="s32" name="s3" value="2" />Quiero liberar el trabajo 

representado en el fichero multimedia</label>
              <label class="btn btn-default btn-block" onclick="$('#i3').show();"><input type="radio" id="s33" name="s3" value="3" />Quiero liberar tanto el trabajo 

representado en el fichero multimedia como el fichero multimedia</label>
            </div>
            <br />
            <p>Si el trabajo multimedia representa o incluye el trabajo artístico de otra persona de una forma no trivial, es un <a 

href="//commons.wikimedia.org/wiki/Commons:Derivative_works" target="_blank"><b>trabajo derivado</b></a> y por lo tanto normalmente es necesaria una liberación 

separada del titular de los derechos de autor de la obra de arte representada.</p>
            <p>En algunos países, gracias a la excepción de la <a href="//commons.wikimedia.org/wiki/Commons:Freedom_of_panorama" target="_blank"><b>libertad de 

panorama</b></a>, obras arquitectónicas y otras obras de arte visibles de forma permanente en el espacio público están exentas de este requisito. En los Estados 

Unidos, sólo está exenta la arquitectura.</p>
            <br /><br />
          </div>
          <div class="col-md-4">
...
+Wikimedia+OTRS+release+generator+step+4" target="_blank">ayuda</a> | <a href="//commons.wikimedia.org/wiki/User_talk:FDMS4?
...
(dominio público)</a></li>
            ...
            <p>Reconozco que haciento esto concedo a cualquiera el derecho a utilizar el trabajo, incluso en un producto comercial o de otra manera, y a modificarlo 

según sus necesidydes, a condición de que mantengan los términos de la licencia y cualquier otras leyes aplicables.</p>
            <p>Soy consciente que este acuerdo no está limitado a Wikipedia o a sus sitios asociados.</p>
            <p id="iawattr">Soy consciente de que el titular de los derechos de autor siempre retiene la propiedad de los derechos así como el derecho de tener la 

atribución de acuerdo con la licencia escogida. Las modificaciones que otros hagan al trabajo no serán solicitadas que fueron hechas por el titular de los derechos de 
...
action=edit&section=new&preloadtitle=help+with+Wikimedia+OTRS+release+generator+step+5" target="_blank">ayuda</a> | <a 
...
&& ($license != "")) {
        ...
        ?>
        <div class="col-md-7">
          <p>Si tiene un cliente de correo electrónico instalado, simplemente <b>pulse el botón</b> para crear el correo de liberación. De forma alternativa, copie y 

pegue el texto de la caja verde de abajo en un correo electrónico para <a href="mailto:permissions-es@wikimedia.org" style="white-space:nowrap;">permissions-

es@wikimedia.org</a>.</p>
          <p>El correo electrónico debe enviarse desde una <b>dirección de correo electrónico que podamos reconocer como asociado con el contenido a ser liberado</b>. 

Por ejemplo, si está liberando imágenes mostradas en un sitio web, su dirección de correo electrónico debe estar asociada con el sitio web o listado en la página de 

contacto del sitio web; si está liberando imágenes en nombre de una organización, su dirección de correo electrónico tiene que ser una dirección de correo electrónico 

oficial de la organización.</p>
          <br />
          <?php
            switch ($s1) {
                case "1":
...
target='_blank'>https://commons.wikimedia.org/wiki/File:" . str_replace(" " , "_", $file) . "</a>";
            ...
            $b3 = "Reconozco que haciendo esto concedo a cualquiera el derecho de utilizar el trabajo, incluso en un producto comercial o de otra manera, y a 

modificarlo según sus necesidades, a condición de que mantengan las condiciones de la licencia y cualquier otras leyes aplicables.";
            ...
            if ($license != "Creative Commons CC0 1.0 Universal (dominio público)") {
                $b5 = "<br />Soy consciente de que el titular del copyright siempre retiene la propiedad de los derechos de autor así como el derecho a ser atribuído 

de acuerdo con la licencia escogida. Las modificaciones que otros hagan al trabajo no serán atribuídas al titular de los derechos de autor.";
                $b5m = "%0ASoy consciente de que el titular del copyright siempre retiene la propiedad de los derechos de autor así como el derecho a ser atribuído de 

acuerdo con la licencia escogida. Las modificaciones que otros hagan al trabajo no serán atribuídas al titular de los derechos de autor.";
            }
...
%0A<?=$name?><?=$p1s_m?>%0A<?=date('Y-m-d')?>%0A%0A<?=$tracking?>" class="btn btn-default btn-block" style="width:100%;">crear correo de liberación</a>
        ...
        <?php
                ...
                if (($s1 == "2") && (($rep == "") || ($auth == ""))) echo "<p class='text-danger'>Error: No se indicó el titular de los derechos de autor y/o 
